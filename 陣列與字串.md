...
### 範例一
```c
void show(int arr[]); /*宣告函數prototype*/
...
void show(int arr[]) /*arr[]表示"陣列"，如果不加[]，可能是非陣列*/
{
    int i;
    for(i=0; i<4; i++)
    {
...
```
## 函數傳遞引數的機制
傳遞一般變數名稱時，接收的函數會將**參數的內容複製一份**，放在函數所使用的記憶體中。
```c
void func(int);
...
{
    int a = 13;
    printf("於main()裡, a = %d, a 的位址 = %p\n", a, &a);
    func(a);
      
    return 0;
}

void func(int A) /*複製a的值，13，然後存放在A裡*/
{
    printf("於func()裡, A = %d, A 的位址 = %p\n", A, &A);
}
```
傳遞陣列時，由於其長度可能很大，基於執行效率的考量，C採取**傳址呼叫**機制: 傳遞到函數中的只是該陣列存放於記憶體的體內位址。(同此陣列的第一個元素的位址)
```c
void func(int a[]);
...
```
換句話來說，其實main()裡的A陣列與func()內的陣列是同一個陣列，只是名字不同。

## 陣列的位置
C以陣列的第一個元素的位址當成陣列的位址。
```c
void func(int a[]);
...
int main()
{
    ...
    printf("arrA的位址=%p\n", A); /*列印出陣列A的位址，注意:陣列名稱本身就是存放陣列位址的變數*/
}
```
### 範例二 傳址機制
```c
