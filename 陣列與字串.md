...
```
bubble sort雖然結果正確，但是儘管資料已完成排序，也必須不斷的重複比較，直到外層迴圈執行完畢為止，較浪費時間。為了改進此缺點，下面範例引進了一個旗標變數(flag)，用來控制進入外層迴圈的時機。
...
```
搜尋過程中，若有2元素調換，則**flag = 0**; 無調換則**flag = 1**，表陣列已完成排序，不用下一回合的搜尋。
