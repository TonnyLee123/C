...
{   
    ...
    // 預設 score[6] = {60, 70, 80, 90, 0, 0, 0, 0, 0, 0}; 
...
# 二維陣列及多維陣列
## 2-dimensions array的宣告與初值的設定
```
資料型態 陣列名稱 [row][column] 

資料型態 陣列名稱 [列的個數][行的個數] = {{第一列初值}, {第二列初值}, {...}}

注意: 橫為row，直為column

二維及二維以上在設定初值時，可省略第一個索引值，可以方便的增減陣列大小。  
...
int main()
{
   int i, j, sale[2][4], sum = 0;
    ...
    for(i = 0; i < 2; i++)                            // 從外圍的元素開始
       ...
       {
           printf("業務員%d的第%d季業績: ", i+1, j+1); // 00, 01, 02, 03, 10, 11, 12, 13
           scanf("%d", &sale[i][j]);
       }
       
    for(i = 0; i < 2; i++)                            /*輸出銷售量並計算總數*/
    {
        printf("\n業務員%d的業績分別為", i + 1);
        for(j = 0; j < 4; j++)
        {
...
```
### 範例三 二維矩陣的相加
數學上的矩陣(matrix)其結構和二維陣列類似，因而可以利用二維陣列儲存矩陣。兩個相同維度的陣列，可以進行加法的運算。  
